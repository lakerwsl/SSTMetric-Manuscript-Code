...
Error <- matrix(0, nrow = 3, ncol = length(TS))
for (j in 1:length(TS))
{
  ...
  Ree<-foreach(t=1:times, .combine=rbind, .packages='class') %dorng% {
    ...
    SelfTrainX=DataAugmentation(x_train[SelfTrainIndex,,],2)
    TrainX=DataTransformation(x_train[TrainIndex,,])
    TestX=DataTransformation(x_test[TestIndex,,])
  ...
  Error[1,j]<-mean(Ree[,1])
  Error[2,j]<-mean(Ree[,2])
  Error[3,j]<-mean(Ree[,3])
}
